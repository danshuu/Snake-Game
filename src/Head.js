...
class Head {
  ...
  constructor($el) {
    this.node = $('<img id="head" src="https://images.all-free-download.com/images/graphiclarge/tom_and_jerry_003_49836.jpg"></img>');
  ...
  move() {
    ...
    let tomHead = $("#tomhead");
    console.log(tomHead.position().left);

    if (position.left === tomHead.position().left && position.top === tomHead.position().top) {
  ...
  outOfBounds(horizontal, vertical) {
    if (horizontal < 0 || horizontal > 950) return true;
